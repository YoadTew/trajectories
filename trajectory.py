...
class App:
    def __init__(self, video_src, output_jpg_path=None, output_pkl_path=None):
        ...
        self.frame_idx = 0
        self.history_tracks = []
        self.output_jpg_path = output_jpg_path
    ...
    def run(self):
        ...
        while True:
            ...
            if len(self.tracks) > 0:
                ...
                for tr, (x, y), good_flag in zip(self.tracks, p1.reshape(-1, 2), good):
                    if not good_flag:
                        self.history_tracks.append(([tr], self.frame_idx - 1))
                        continue
            ...
            print("curr frame is", self.frame_idx)
            self.frame_idx += 1
        ...
        with open(self.output_pkl_path, 'wb') as f:
            pickle.dump(self.history_tracks, f)
        print("Drawing...")
        cv2.polylines(frame, [np.int32(tr) for tra in draw_history_tracks for tr in tra], False, (0, 255, 0))
        print("Saving to jpeg...")
        if self.output_jpg_path is not None:
            cv2.imwrite(self.output_jpg_path, frame)
        return self.history_tracks


def load_pickle(output_pkl_path):
    import pickle
...
def main():
    ...
    output_jpg_path = '/home/work/Lktrack/lk_track_enddd.jpg'
    output_pkl_path = '/tmp/history.obj'
    App(video_src, output_jpg_path, output_pkl_path).run()
